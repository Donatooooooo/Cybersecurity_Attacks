...
def browser(userAgent):
    browserPattern = re.compile(
        r"Mozilla.*?(Firefox|Chrome|MSIE|Safari)", re.IGNORECASE
    )
...
def os(userAgent):
    osPattern = re.compile(
        r"(Windows|Mac OS|Linux|iPhone OS|iPad OS|iPod OS|Android)", re.IGNORECASE
    )
...
def basicPreprocessing(dataset: Dataset):
    ...
    try:
        data["Browser"] = data["Device Information"].apply(
            lambda x: browser(x) if pandas.notnull(x) else None
        )
        data["OS"] = data["Device Information"].apply(
            lambda x: os(x) if pandas.notnull(x) else None
        )
...
def prologPreprocessor(path):
    ...
    dataset["Proxy Information"] = dataset["Proxy Information"].apply(
        lambda x: "Proxy" if pandas.notna(x) else "None"
    )

    mean = dataset["Packet Length"].mean()
    dataset["Packet Length"] = dataset["Packet Length"].apply(
        lambda x: "Long" if x > mean else "Short"
    )

    try:
        dataset["OS"] = dataset["Device Information"].apply(
            lambda x: os(x) if pandas.notnull(x) else None
        )
    except userAgentException as e:
        print(e)
